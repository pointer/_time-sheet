        ...
        <v-card-text>
          ...
          <v-row no-gutters>
            ...
            <v-col class="mt-4 pr-6">
              <v-text-field clearable v-model="user.password" :append-inner-icon="visible ? 'mdi-eye-off' : 'mdi-eye'"
                required :type="visible ? 'text' : 'password'" density="compact" placeholder="Enter your password"
                :rules="[min6]" :counter="6" prepend-inner-icon="mdi-lock-outline" variant="underlined"
            ...
            <!-- <v-col cols="12" md="6"> -->
            <v-col class="mt-4">
              <v-text-field clearable v-model="passConfirm" :append-inner-icon="visible ? 'mdi-eye-off' : 'mdi-eye'"
                required :type="visible ? 'text' : 'password'" density="compact" placeholder="Confirm your password"
                :rules="[min6, matchingPasswords]" :counter="6" prepend-inner-icon="mdi-lock-outline"
                variant="underlined">
                <template v-slot:append>
          ...
          <v-row no-gutters>
            ...
            <v-col cols="12" md="6">
              <v-text-field clearable v-model="user.companyId" color="primary" label="Company ID"
                variant="underlined"></v-text-field>
            </v-col>
            <v-col cols="12" md="6">
            ...
            <v-col cols="12" md="6">
              <v-combobox clearable chips v-model="user.project" :items="projects" label="Project"
                required></v-combobox>
            </v-col>
            <v-col cols="12" md="6">
              <v-combobox clearable chips :items="['Contract', 'Supervisor', 'Employee']" v-model="user.role" required
                label="Role"></v-combobox>
            </v-col>
            <v-col cols="12" md="6">
          ...
          <v-row no-gutters>
            <v-col cols="12" md="6">
              <v-text-field clearable v-model="user.dateStart" type="date" color="primary" label="Start Date" required
                variant="underlined"></v-text-field>
            </v-col>
            <v-col cols="12" md="6">
              <v-text-field clearable v-model="user.dateEnd" type="date" color="primary" label="End Date" required
                variant="underlined"></v-text-field>
            </v-col>
          </v-row>
...
const passRule = () => {
  if (user.password === '') {
    // field is empty
    user.successPass = false;
    return '';
  } else if (min6(user.password) === true) {
    // password ok
    successPass = true;
    return 'mdi-check';
  } else {
...
const passRule1 = () => {
  if (passConfirm === '') {
    ...
    successPass1 = false;
    return '';
  } else if (min6(passConfirm) === true && matchingPasswords() === true) {
    // password ok
    successPass1 = true;
    return 'mdi-check';
  } else {
