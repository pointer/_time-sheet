...
export default {
  getTimesheetsByMonth(month, approver, token) {
      return axios.get(`${baseUrl}/api/approbations/${month}/${approver}`, {
          headers: {
...
saveApprobation(token, payload) {
  ...
  saveApprobations(payload) {
    console.log(payload)
    return axios.post(`${import.meta.env.VITE_SRV_API_URL}/api/approbations`, payload, {
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`
    }
  }).catch(error => {
    console.error('Error in saveApprobations:', error.response ? error.response.data : error);
    throw error;
  });
},
